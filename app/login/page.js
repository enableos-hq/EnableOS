      ...
      <div style={{ width: '100%', maxWidth: 420, position: 'relative' }}>
        ...
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, border: `1px solid ${S.border}`, boxShadow: '0 4px 32px rgba(124,92,252,0.08)' }}>
          <form onSubmit={handle}>
            <div style={{ marginBottom: 16 }}>
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@company.com" required
                style={{ width: '100%', padding: '12px 16px', border: `1px solid ${S.border}`, borderRadius: 8, fontSize: 14, fontFamily: 'var(--font-body)', outline: 'none', color: S.ink, boxSizing: 'border-box' }}
            ...
            {error && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: S.error }}>{error}</div>}
            {success && <div style={{ background: '#d1fae5', border: '1px solid #6ee7b7', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 13, color: '#065f46' }}>{success}</div>}
            <button type="submit" disabled={loading}
              style={{ width: '100%', padding: '13px', background: S.ink, color: '#fff', border: 'none', borderRadius: 10, fontSize: 15, fontWeight: 700, fontFamily: 'var(--font-body)', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, opacity: loading ? 0.7 : 1 }}>
          ...
          <div style={{ textAlign: 'center', marginTop: 20, paddingTop: 20, borderTop: `1px solid ${S.border}` }}>
            <span style={{ fontSize: 14, color: S.muted }}>{mode === 'login' ? "Don't have an account? " : 'Already have an account? '}</span>
